...
void wait (int ms, webots::Robot* myRobot, int timeStep) {
    ...
    while (s+start >= myRobot->getTime())
        myRobot->step(timeStep);
}

int main() {
    std::cout << "Test Soccer" << std::endl;
    ...
    while(true) {
        ...
        while((key = keyboard->getKey()) >= 0) {
            ...
            switch (key) {
                case ' ':
                    std::cout << "is walking " << walking->IsRunning() << std::endl;
                    std::cout << "x move amplitude " << walking->X_MOVE_AMPLITUDE << std::endl;
                    if (isWalking) {
                        walking->Stop();
                        isWalking = false;
